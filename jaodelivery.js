...
client.on('message', async msg => {
    ...
    if (msg.body.startsWith('jao: ')){
        client.sendMessage(WHATSAPP_LICENCIADO + '@c.us', 'Ok! Disparando mensagens...');

        const mensagem = msg.body.slice(5);
        ...
        clientSheet.read().then(function(data){
            const respostas = JSON.parse(data);
                respostas.forEach((resposta, i) => {
                    ...
                    const bodyMessage = mensagem.replace('{nome}', resposta.social);
                    setTimeout(function(){
                        ...
                        client.sendMessage(whatsapp + '@c.us', bodyMessage);
                        
                    }, 1000 + Math.floor(Math.random() * 8000) * (i+1));
                    cont++;
                });
                
            client.sendMessage(WHATSAPP_LICENCIADO + '@c.us', '*Relatório de envio:* Foram enviadas '+ cont +' mensagems ao todo!');     
        });
